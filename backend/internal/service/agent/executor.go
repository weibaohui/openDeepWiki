...
func (e *Executor) Execute(ctx context.Context, repoPath string, outputPath string) error {
	...
	// 1. 代码探索阶段 - 类似 ExplorerAgent
	analysis, err := e.exploreCode(ctx, repoPath)
	if err != nil {
		klog.Errorf("代码探索失败: %v", err)
		return fmt.Errorf("代码探索失败: %w", err)
	}
	klog.V(6).Infof("代码探索完成: projectType=%s, language=%s", analysis.ProjectType, analysis.Language)
...
// exploreCode 代码探索 - 实现ExplorerAgent功能
