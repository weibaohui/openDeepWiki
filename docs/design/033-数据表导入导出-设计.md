...
# 6. 前端交互设计
- 新增同步模式选择（Push/Pull），并增加提醒框，解释选择的这个模式，会发生什么，用概要的箭头表示，以谁为准，同步方向，是否会清空对端数据。
  - Push 模式：实例 A 发起同步，先保证实例 B 存在对应 Repository，再按 Task → Document → Task 更新 DocumentID 的顺序逐条同步。
  - Pull 模式：实例 A 从实例 B 拉取数据，先从实例 B 获取仓库与文档列表，导出数据后在实例 A 执行 Repository、Task、Document 落库与映射更新。
  - 画一个简单的字符串性质的图示，展示 Push 模式与 Pull 模式的同步方向。
- 同步表单增加目标服务器与仓库选择
- 同步表单增加文档下拉表格选择
