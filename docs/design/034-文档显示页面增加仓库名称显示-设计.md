...
## 1. 概述
在文档显示页左侧边栏顶部和文档概览页内容区域顶部增加仓库名称显示，并在仓库名称下方提供跳转和复制图标按钮，帮助用户识别文档来源仓库，并方便访问和分享仓库地址。同时确保响应式设计，在桌面端和移动端都能正常显示和使用。
...
| DocViewer 页面 Content | 在概览页内容区域顶部增加仓库名称和图标按钮 |
...
const SidebarContent = () => (
    <>
        ...
        <div style={{
            ...
            {repository?.name && (
                <div>
                    <Text strong style={{ fontSize: '16px', display: 'block', marginBottom: 6 }}>
                        {repository.name}
                    </Text>
                    {repositoryUrl && (
...
### 6.2 概览页内容区域
在 DocViewer.tsx 的 `isIndexView` 条件中，在统计数据之前增加仓库名称和图标按钮：
