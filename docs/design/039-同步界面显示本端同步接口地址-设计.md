...
在同步页面提供历史记录抽屉,倒序展示同步记录,支持过滤与刷新。
...
**入口位置:** Sync 页面 Header 区域右侧按钮  
**交互流程:** 点击按钮 → 打开抽屉 → 自动加载历史记录 → 可筛选与刷新  
**表格字段:** 时间、模式、目标服务器、仓库、文档、结果  
**过滤能力:** 仓库过滤与模式过滤  
**刷新策略:** 提供手动刷新按钮,抽屉打开时定时刷新

**数据来源:**
- 后端新增同步事件列表接口,按时间倒序返回
- 支持按仓库与模式过滤
