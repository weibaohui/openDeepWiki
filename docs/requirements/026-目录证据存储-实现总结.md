...
## 1. 实现概述
目录生成流程已支持结构化证据输出与落库，证据与任务、仓库关联保存，便于后续文档生成定位标题依据。
...
| 需求点 | 实现情况 | 说明 |
| :--- | :--- | :--- |
| 目录输出包含证据列表 | 已完成 | 目录解析结构支持 evidence |
| 证据独立表存储 | 已完成 | 新增 TaskEvidence 模型与表 |
| 证据与任务关联 | 已完成 | 保存时写入 task_id 与 repository_id |
| 目录生成流程保存证据 | 已完成 | 创建任务后批量保存证据 |

## 3. 关键实现点

### 3.1 模型与仓储
- 新增 TaskEvidence 模型并注册数据库迁移。
- 新增 EvidenceRepository 支持批量保存与按任务查询。

### 3.2 目录分析流程
- dirmaker 解析 evidence 字段并按任务保存。
- 保存失败仅记录告警，不影响任务创建。
...
2. **数据泄露**：证据仅记录目录与配置来源，不记录本地路径。
3. **输入处理**：证据内容来自 Agent 输出，落库前无执行型处理。

## 6. 变更文件清单
- `backend/internal/model/models.go`
- `backend/internal/pkg/database/database.go`
- `backend/internal/repository/repository.go`
- `backend/internal/repository/evidence_repo.go`
- `backend/internal/repository/evidence_repo_test.go`
