...
## 1. 实现概述
实现数据库模型解析服务，基于 TaskEvidence 线索构建 YAML 输入，使用“探索 Agent + markdown_checker”顺序生成 Markdown 文档，并由前端目录分析按钮触发后端接口异步生成文档保存到 documents 表。
...
| 需求点 | 实现情况 | 说明 |
| :--- | :--- | :--- |
| 证据筛选与 YAML 输入 | 已完成 | 仅保留数据库相关证据，输出 title/detail/source |
...
### 3.1 证据筛选与 YAML
从 TaskEvidence 过滤数据库相关关键词，仅保留标题、来源与细节，并序列化为 evidences 列表。
