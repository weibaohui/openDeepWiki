...
| ------ | -------- | -------- |
| AI | 2026-02-08 | 初始版本 |
| AI | 2026-02-08 | 补充任务类型约定 |
| AI | 2026-02-08 | 补充前端触发与接口说明 |
| AI | 2026-02-08 | 补充仓库范围证据检索与接口签名 |

# 1. 背景（Why）
当前文档生成流程缺少针对数据库结构的专项解析能力，无法系统化整理表结构与字段含义。目录分析阶段已经沉淀了 TaskHint，但没有被用于数据库结构类文档的进一步扩展与深挖，因此需要新增数据库模型解析服务，基于证据线索生成高质量的表结构说明文档。

# 2. 目标（What，必须可验证）
- [ ] 从 TaskHint 中筛选数据库相关证据，并序列化为 YAML 输入
...
# 4. 使用场景 / 用户路径
1. 前端 RepoDetail 页面存在两个独立按钮：
   - “目录分析”按钮：仅触发目录分析接口
   - “数据库模型分析”按钮：仅触发数据库模型分析接口
2. 目录分析任务中可包含数据库模型任务，但数据库模型分析也可独立触发
3. 独立接口：`POST /api/repositories/:id/db-model-analyze`
4. 服务在仓库范围内检索 TaskHint 中数据库相关线索
5. 组装 YAML 输入并调用探索 Agent
6. markdown_checker 校验输出
7. 保存文档到 documents 表并返回结果
...
- 任务类型：
  - db-model
- 任务类型规范化：
  - database-model/data-model/db_model/database_model/data_model -> db-model
- YAML 输入结构：
  - hints:
    - title: "<证据标题>"
      detail: "<证据细节>"
...
- 如果 TaskHint 包含数据库相关条目，则 YAML 输入包含对应条目
