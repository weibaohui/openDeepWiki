...
| 修改人 | 修改时间 | 修改内容 |
| ------ | -------- | -------- |
| AI | 2026-02-10 | 增加同步文档选择控件与参数说明 |
| AI | 2026-02-10 | 增加清空对端同步选项与流程说明 |
| AI | 2026-02-13 | 增加多实例间数据 pull 同步需求与流程 |
...
**Pull 模式典型使用流程**：
1. 用户在实例 A 上打开同步页面并选择 Pull 模式
2. 在"远端服务器"输入框中填写实例 B 的同步接口地址（如：`http://b-server/api/sync`）
3. 系统从实例 B 加载可同步的仓库列表
4. 在"待同步仓库"下拉框中选择要拉取的代码仓库（如：`repo-xxxx`）
5. （可选）在"待同步文档"中勾选要拉取的文档
6. 点击"开始同步"按钮
7. 系统从实例 B 拉取该仓库的 Task 和 Document 数据同步到实例 A
8. 显示同步进度和结果
...
**接口 0.5：Repository 清空接口**
- **路径**：`POST /api/sync/repository-clear`
- **功能**：按 repository_id 清空接收端 Task、Document 数据
- **请求参数**：
  ```json
  {
    "repository_id": "80"
...
#### 3.1.3 Pull 同步接口（拉取端 - 实例 A / 数据源端 - 实例 B）

**接口 6：远端仓库列表接口（数据源端 - 实例 B）**
- **路径**：`GET /api/sync/repository-list`
- **功能**：返回实例 B 可同步的仓库列表，用于 Pull 模式的仓库选择
- **响应**：
  ```json
  {
    "code": "OK",
...
**接口 7：远端文档列表接口（数据源端 - 实例 B）**
- **路径**：`GET /api/sync/document-list?repository_id=80`
- **功能**：返回指定仓库的文档列表，用于 Pull 模式的文档多选
- **响应**：
  ```json
  {
    "code": "OK",
...
**接口 8：远端数据导出接口（数据源端 - 实例 B）**
- **路径**：`POST /api/sync/pull-export`
- **功能**：按 repository_id 与 document_ids 导出 Task 与 Document 数据，供 Pull 模式拉取
- **请求参数**：
  ```json
  {
    "repository_id": "80",
...
3. 同步 Repository 基本信息到实例 B
   ↓
4. 若传入 document_ids，则仅同步指定 Document
5. 若 clear_target=true，则先按 repository_id 清空对端 Task、Document 数据
   ↓
6. 查询指定 repository_id 的所有 Task
7. 对每个 Task（按顺序）：
   6.1 调用 B 的 /api/sync/task-create
   6.2 调用 B 的 /api/sync/document-create
   6.3 调用 B 的 /api/sync/task-update-docid
   6.4 更新同步进度
   ↓
8. 全部同步完成后，返回最终结果
```
...
4. 用户选择仓库与可选的文档列表
   ↓
5. 若 clear_local=true，则先按 repository_id 清空本地 Task、Document 数据
   ↓
6. 调用实例 B 的 /api/sync/pull-export 导出数据
7. 在实例 A 创建/更新 Repository
8. 对每个 Task 与 Document（按顺序）：
   8.1 创建 Task（不包含 DocumentID）
   8.2 创建 Document（关联 Task）
   8.3 更新 Task 的 DocumentID
   8.4 更新同步进度
   ↓
9. 全部同步完成后，返回最终结果
```
...
- 待同步文档下拉表格（支持多选，不选时默认全部）
  - 标题列支持点击跳转 `/repo/x/doc/y`
  - 状态列展示关联任务状态
  - 创建时间列展示文档创建时间
- Pull 模式下仓库列表从远端实例 B 加载，并支持刷新
- 清空对端数据选项。在push模式下，勾选后按 repository_id 删除对端 Task、Document。在pull模式下，勾选后按repository_id 删除本地的 Task、Document
- 开始同步按钮
- 同步进度条
