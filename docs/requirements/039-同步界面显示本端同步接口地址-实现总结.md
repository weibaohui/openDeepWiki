...
| ------ | -------- | -------- |
| AI | 2026-02-12 | 初始版本 |
| AI | 2026-02-15 | 增加同步历史记录抽屉展示 |

## 一、功能概述

本次实现为 openDeepWiki 的数据同步功能添加了本端同步接口地址展示功能,并在同步页面新增同步历史记录抽屉,支持筛选与刷新。
...
| 4.5 同步历史记录 | 抽屉表格展示同步记录,支持筛选与刷新 | ✅ 已实现 | 新增历史记录抽屉与查询接口 |
...
#### 3.1.5 同步历史记录展示
同步页面新增历史记录抽屉,通过 `sync/event-list` 接口拉取同步记录,支持仓库与模式筛选并定时刷新:
...
const fetchHistoryEvents = useCallback(async () => {
    ...
    setHistoryEvents(response.data.data);
}, [historyMode, historyRepositoryId]);
```
...
| 文件路径 | 修改类型 | 修改内容 |
| -------- | -------- | -------- |
| `frontend/src/pages/Sync.tsx` | 功能新增 | 添加本端同步接口地址展示与同步历史记录抽屉 |
| `frontend/src/services/api.ts` | 功能新增 | 增加同步历史记录查询接口 |
| `frontend/src/types/index.ts` | 类型新增 | 增加同步历史记录类型 |
| `frontend/src/i18n/locales/zh-CN.json` | 数据修改 | 添加同步历史记录相关中文翻译 |
| `frontend/src/i18n/locales/en-US.json` | 数据修改 | 添加同步历史记录相关英文翻译 |
| `backend/internal/handler/sync.go` | 功能新增 | 增加同步历史记录查询接口 |
| `backend/internal/service/sync/service.go` | 功能新增 | 增加同步记录查询服务 |
| `backend/internal/repository/sync_event_repo.go` | 功能新增 | 增加同步事件列表查询 |
| `backend/internal/dto/sync/sync.go` | 类型新增 | 增加同步事件列表 DTO |
...
```bash
cd /Users/weibh/projects/go/openDeepWiki/backend && go test ./...
```
✅ 后端测试通过
...
## 七、安全自检
1. 同步历史记录接口仅返回同步事件元数据,未包含敏感凭据
2. 接口支持筛选条件校验,不允许非法模式值进入查询
3. 未新增高权限操作或越权访问路径
